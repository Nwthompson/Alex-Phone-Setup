...
HWC3::Error Display::getDisplayCapabilities(std::vector<DisplayCapability>* outCapabilities) {
    ...
    // Virtio-gpu (minigbm) does not apply a host display color transform. Leave
    // the transform to SurfaceFlinger so it is baked into the posted framebuffer.
    const bool virtioGpu = ::android::base::GetProperty("ro.hardware.gralloc", "") == "minigbm";
    if (!virtioGpu) {
